...
RUN wget -O - https://dl.winehq.org/wine-builds/winehq.key | gpg --dearmor | sudo tee /etc/apt/keyrings/winehq-archive.key
...
RUN sudo mv winehq-$(lsb_release -sc).sources /etc/apt/sources.list.d/
