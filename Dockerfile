...
# Create a command to launch MetaTrader 4 on a virtual screen
RUN echo '#!/bin/sh' >> /usr/local/bin/launch-mt4 && \
    echo 'Xvfb :1 -screen 0 1024x768x16 & sleep 5 && DISPLAY=:1 wine "C:.\mt4\ mt4setup.exe"' >> /usr/local/bin/launch-mt4 && \
